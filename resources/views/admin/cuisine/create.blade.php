        ...
        <div class="row">
            <div class="col-xs-12">
                <div class="box box-primary">
                    ...
                    {{ Form::open(['url' => route('cuisine.store'), 'method'=>'POST', 'files'=>true, 'name' => 'frmCuisine', 'id' => 'frmCuisine']) }}
                     <div class="box-body">
                        <div class="row"> 
                            <div class="form-group col-sm-6">
                                {{ Form::label('name','Name') }}
                                {{ Form::text('name', old('name'), ["required","class"=>"form-control","placeholder"=>'Enter Cuisine Name',"id"=>"name","name"=>"name","maxlength"=>100]) }}
                            </div>
                            
                            <div class="form-group col-sm-3">
                                {{ Form::label('status','Status') }}                                     
                                {{ Form::select('status',["A"=>"ACTIVE","I"=>"INACTIVE"], old('status'),["required","class"=>"form-control","style"=>"width:100%","id"=>"status","name"=>"status"]) }}
                            </div>                             
                        </div>                        
                    </div>
