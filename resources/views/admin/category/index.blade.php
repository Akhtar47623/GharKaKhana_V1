            ...
            <div class="col-xs-12">
                <div class="box box-primary">
                    <div class="box-header with-border content-header">
                        <h3>{{ $title }}  
                            @if($permission::checkActionPermission('create_category'))
                            <a href="{{ route('category.create') }}" class="btn btn btn-primary" title="Add new"><i class="fa fa-plus-circle"></i> ADD</a>
                            @endif
                            ...
                            data-action="{{ route('categorymultidelete') }}"
                ...
                <div class="box-body">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><img class="img-responsive" height="50px" width="50px" src="{{asset('public/backend/images/category/'.$value->image)}}" /></td>
                            <td>{{$value->name}}</td>
                            <td>{{$value->country_id}}</td>
                            <td>{{$value->state_id!=NULL?$value->state_id:'ALL'}}</td>
                            <td class="text-right">
                                @if($permission::checkActionPermission('update_category'))
                                <a href="{{route('category.edit', $value->id)}}" title="Edit" class="btn btn-round btn-warning"><i class="fa fa-pencil"></i></a>
                                @endif
                                @if($permission::checkActionPermission('delete_category'))
                                <button id="country" type="button" title="Delete" rel="tooltip" class="btn  btn-danger remove"  data-id="{{($value->id)}}"  data-action="{{route('category.destroy', $value->id)}}" data-message="{{Config::get('constants.message.confirm')}}"><i class="fa fa-trash"></i></button>
                                @endif
                            </td>
                        @endforeach
                        @endif
