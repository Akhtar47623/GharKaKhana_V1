            ...
            <div class="col-xs-12">
                <div class="box box-primary">
                    <div class="box-header with-border content-header">
                        <h3>{{ $title }}  
                            @if($permission::checkActionPermission('create_vendor_discount'))
                            <a href="{{ route('vendor-discount.create') }}" class="btn btn btn-primary" title="Add new"><i class="fa fa-plus-circle"></i> ADD</a>
                            @endif
                            @if($permission::checkActionPermission('delete_vendor_discount'))
                            ...
                            data-action="{{ route('vendordiscountmultidelete') }}"
                ...
                <div class="box-body">
                    ...
                    <tbody>
                        @if(!empty($vendordiscountData))
                        @foreach ($vendordiscountData as $value)
                        <tr>
                            <td>
                                <div class="form-check pull-left">
                                    <label class="form-check-label">
                            ...
                            <td class="text-right">
                                @if($permission::checkActionPermission('update_vendor_discount'))
                                <a href="{{route('vendor-discount.edit', $value->id)}}" title="Edit" class="btn btn-round btn-warning"><i class="fa fa-pencil"></i></a>
                                @endif
                                @if($permission::checkActionPermission('delete_vendor_discount'))
                                <button id="country" type="button" title="Delete" rel="tooltip" class="btn  btn-danger remove"  data-id="{{($value->id)}}"  data-action="{{route('vendor-discount.destroy', $value->id)}}" data-message="{{Config::get('constants.message.confirm')}}"><i class="fa fa-trash"></i></button>
                                @endif
                            </td>
...
<script>
  $(function() {
    ...
    var vendor_discount = $(this).data('id');
    $.ajax({
      type: "GET",
      dataType: "json",
      url: 'vendor-discount-changestatus',
      data: {'status': status, 'vendor_discount': vendor_discount},
